...
<body class="bg-brand-lightPink/50 font-sans text-brand-dark antialiased min-h-screen flex flex-col justify-between">
    ...
    <main class="max-w-5xl mx-auto w-full px-6 py-16 flex-grow space-y-12">
        ...
        <div class="border-b border-gray-200 pb-6 mb-10">
            <h1 class="font-serif text-3xl font-bold tracking-tight text-brand-dark">Clinical Profiles & Accreditations</h1>
            <p class="text-xs text-brand-textMuted mt-1">Verified background logs and structural core competencies of our resident medical team.</p>
        </div>

        <?php if (empty($doctors)): ?>
        <div class="text-center py-20">
            <i class="fa-regular fa-user-xmark text-4xl text-gray-300 mb-4 block"></i>
            <p class="text-sm text-gray-400 font-medium">No doctor profiles available at the moment.</p>
        </div>
        <?php else: ?>
        <div class="space-y-8">
            <?php foreach ($doctors as $doctor): ?>
            <section class="bg-white border border-gray-100 rounded-3xl p-6 md:p-8 shadow-[0_15px_40px_rgba(0,0,0,0.01)] grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
                ...
                <div class="md:col-span-8 lg:col-span-9 flex flex-col justify-between h-full min-h-[180px] space-y-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            <h2 class="font-serif text-2xl font-bold tracking-tight text-brand-dark mt-0.5">Dr. <?php echo htmlspecialchars($doctor['name']); ?></h2>
                            <?php if (!empty($doctor['email'])): ?>
                            <span class="text-[10px] text-gray-400 font-medium block mt-0.5"><?php echo htmlspecialchars($doctor['email']); ?></span>
                            <?php endif; ?>
                        </div>

                        <?php if (!empty($doctor['description'])): ?>
                        <div class="space-y-1">
                            <h4 class="text-[10px] font-bold uppercase tracking-wider text-gray-400">Clinical Focus & Biography</h4>
                    ...
                    <div class="flex items-center space-x-3 pt-2 max-w-md">
                        <div class="flex-1 bg-slate-50 border border-slate-100 rounded-xl p-2.5 text-center">
                            <span class="text-[8px] font-bold text-gray-400 uppercase tracking-widest block mb-0.5">Experience</span>
                            <span class="text-[11px] font-bold text-brand-dark"><?php echo $doctor['experience']; ?>+ Yrs Active</span>
                        </div>
                        
                        <?php if (!empty($doctor['phone'])): ?>
                        <a href="tel:<?php echo htmlspecialchars($doctor['phone']); ?>" class="flex-1 bg-brand-lightPink/50 border border-pink-100/40 rounded-xl p-2.5 text-center hover:bg-brand-lightPink transition-colors block">
                        ...
                        <?php else: ?>
                        <div class="flex-1 bg-gray-50 border border-gray-100 rounded-xl p-2.5 text-center opacity-50">
